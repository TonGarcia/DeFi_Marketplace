...
module.exports = {
  ...
  solc_args: ["--allow-paths *,"],                          // Extra solc args
  solc_shell_args: {                                       // Args passed to `exec`, see:
    maxBuffer: 1024 * 150000                                // https://nodejs.org/api/child_process.html#child_process_child_process_spawn_command_args_options
  },
  ...
  //contracts: "contracts/*.sol",                            // Glob to match contract files contracts: "contracts/*.sol",
  contracts: "tests/Contracts/*.sol",                            // Glob to match contract files contracts: "contracts/*.sol",
  ...
  networks: {                                           // Define configuration for each network
    ...
    test: {
      providers: [
        {env: "PROVIDER"},
        {ganache: {gasPrice: 0}},                                  // In test mode, connect to a new ganache provider. Any options will be passed to ganache
      ],
      web3: {
        gas: [
          {env: "GAS"},
