...
interface EIP20Interface {
    ...
    /**
      ...
      * @return success whether or not the transfer 
      */
 ...
 * @title Exponential module for storing fixed-precision decimals
 * @author Niural based on prev readme author
 * @dev Legacy contract for niuatibility reasons with existing contracts that still use MathError
 * @notice Exp is a struct which stores decimals with a fixed precision of 18 decimal places.
 *         Thus, if we wanted to store the 5.1, mantissa would store 5.1e18. That is:
 *         `Exp({mantissa: 5100000000000000000})`.
 */
...
contract NiutrollerErrorReporter {
    enum Error {
        NO_ERROR,
        UNAUTHORIZED,
        NIUTROLLER_MISMATCH,
...
contract TokenErrorReporter {
    enum Error {
        ...
        UNAUTHORIZED,
        BAD_INPUT,
        NIUTROLLER_REJECTION,
        NIUTROLLER_CALCULATION_ERROR,
    ...
    enum FailureInfo {
        ...
        BORROW_NIUTROLLER_REJECTION,
        LIQUIDATE_ACCRUE_BORROW_INTEREST_FAILED,
        LIQUIDATE_ACCRUE_COLLATERAL_INTEREST_FAILED,
        LIQUIDATE_COLLATERAL_FRESHNESS_CHECK,
        LIQUIDATE_NIUTROLLER_REJECTION,
        LIQUIDATE_NIUTROLLER_CALCULATE_AMOUNT_SEIZE_FAILED,
        ...
        LIQUIDATE_SEIZE_NIUTROLLER_REJECTION,
        LIQUIDATE_SEIZE_LIQUIDATOR_IS_BORROWER,
        LIQUIDATE_SEIZE_TOO_MUCH,
        MINT_ACCRUE_INTEREST_FAILED,
        MINT_NIUTROLLER_REJECTION,
        ...
        REPAY_BORROW_NIUTROLLER_REJECTION,
...
// File: InterestRateModelInterface.sol
...
/**
  * @title Niural's InterestRateModel Contract
  * @author Niural
  * @notice The parameterized model is described in section X.y of the original Niural Protocol whitepaper
  */
...
contract NTokenStorage {
     ...
     */
    NiutrollerInterface public niutroller;
...
contract NTokenInterface is NTokenStorage {
    ...
    /**
     * @notice Event emitted when niutroller is changed
     */
    event NewNiutroller(NiutrollerInterface oldNiutroller, NiutrollerInterface newNiutroller);
    ...
    function _setNiutroller(NiutrollerInterface newNiutroller) public virtual returns (uint);
...
abstract contract NErc20Interface is NErc20Storage {
    ...
    function liquidateBorrow(address borrower, uint repayAmount, NTokenInterface NTokenCollateral) external virtual returns (uint);
...
abstract contract NDelegatorInterface is NDelegationStorage {
    /**
...
abstract contract NDelegateInterface is NDelegationStorage {
    /**
...
/**
 * @title Niural's NToken Contract
 * @notice Abstract base for NTokens
 * @author Niural
 */
abstract contract NToken is NTokenInterface, Exponential, TokenErrorReporter {
    /**
     ...
     */
    function initialize(NiutrollerInterface niutroller_,
        ...
        // Set the niutroller
        uint err = _setNiutroller(niutroller_);
        require(err == uint(Error.NO_ERROR), "setting niutroller failed");

        // Initialize block number and borrow index (block number mocks depend on niutroller being set)
    ...
    function transferTokens(address spender, address src, address dst, uint tokens) internal returns (uint) {
        ...
        uint allowed = niutroller.transferAllowed(address(this), src, dst, tokens);
        if (allowed != 0) {
            return failOpaque(Error.NIUTROLLER_REJECTION, FailureInfo.TRANSFER_NIUTROLLER_REJECTION, allowed);
        }
        ...
        // unused function
        // niutroller.transferVerify(address(this), src, dst, tokens);
     ...
     * @dev This is used by niutroller to more efficiently perform liquidity checks.
    ...
    function mintFresh(address minter, uint mintAmount) internal returns (uint, uint) {
        ...
        uint allowed = niutroller.mintAllowed(address(this), minter, mintAmount);
        if (allowed != 0) {
            return (failOpaque(Error.NIUTROLLER_REJECTION, FailureInfo.MINT_NIUTROLLER_REJECTION, allowed), 0);
        }
        ...
        // unused function
        // niutroller.mintVerify(address(this), minter, vars.actualMintAmount, vars.mintTokens);
    ...
    function redeemFresh(address payable redeemer, uint redeemTokensIn, uint redeemAmountIn) internal returns (uint) {
        ...
        uint allowed = niutroller.redeemAllowed(address(this), redeemer, vars.redeemTokens);
        if (allowed != 0) {
            return failOpaque(Error.NIUTROLLER_REJECTION, FailureInfo.REDEEM_NIUTROLLER_REJECTION, allowed);
        }
        ...
        /* We call the defense hook */
        niutroller.redeemVerify(address(this), redeemer, vars.redeemAmount, vars.redeemTokens);
    ...
    function borrowFresh(address payable borrower, uint borrowAmount) internal returns (uint) {
        ...
        uint allowed = niutroller.borrowAllowed(address(this), borrower, borrowAmount);
        if (allowed != 0) {
            return failOpaque(Error.NIUTROLLER_REJECTION, FailureInfo.BORROW_NIUTROLLER_REJECTION, allowed);
        }
        ...
        /* We call the defense hook */
        // unused function
        // niutroller.borrowVerify(address(this), borrower, borrowAmount);
    ...
    function repayBorrowFresh(address payer, address borrower, uint repayAmount) internal returns (uint, uint) {
        ...
        uint allowed = niutroller.repayBorrowAllowed(address(this), payer, borrower, repayAmount);
        if (allowed != 0) {
            return (failOpaque(Error.NIUTROLLER_REJECTION, FailureInfo.REPAY_BORROW_NIUTROLLER_REJECTION, allowed), 0);
        }
        ...
        /* We call the defense hook */
        // unused function
        // niutroller.repayBorrowVerify(address(this), payer, borrower, vars.actualRepayAmount, vars.borrowerIndex);
    ...
    function liquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, NTokenInterface NTokenCollateral) internal returns (uint, uint) {
        ...
        uint allowed = niutroller.liquidateBorrowAllowed(address(this), address(NTokenCollateral), liquidator, borrower, repayAmount);
        if (allowed != 0) {
            return (failOpaque(Error.NIUTROLLER_REJECTION, FailureInfo.LIQUIDATE_NIUTROLLER_REJECTION, allowed), 0);
        }
        ...
        (uint amountSeizeError, uint seizeTokens) = niutroller.liquidateCalculateSeizeTokens(address(this), address(NTokenCollateral), actualRepayAmount);
        require(amountSeizeError == uint(Error.NO_ERROR), "LIQUIDATE_NIUTROLLER_CALCULATE_AMOUNT_SEIZE_FAILED");
        ...
        /* We call the defense hook */
        // unused function
        // niutroller.liquidateBorrowVerify(address(this), address(NTokenCollateral), liquidator, borrower, actualRepayAmount, seizeTokens);
    ...
    function seizeInternal(address seizerToken, address liquidator, address borrower, uint seizeTokens) internal returns (uint) {
        ...
        uint allowed = niutroller.seizeAllowed(address(this), seizerToken, liquidator, borrower, seizeTokens);
        if (allowed != 0) {
            return failOpaque(Error.NIUTROLLER_REJECTION, FailureInfo.LIQUIDATE_SEIZE_NIUTROLLER_REJECTION, allowed);
        }
        ...
        /* We call the defense hook */
        // unused function
        // niutroller.seizeVerify(address(this), seizerToken, liquidator, borrower, seizeTokens);
    ...
    /**
      * @notice Sets a new niutroller for the market
      * @dev Admin function to set a new niutroller
      * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
      */
    function _setNiutroller(NiutrollerInterface newNiutroller) public override returns (uint) {
        // Check caller is admin
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_NIUTROLLER_OWNER_CHECK);
        }

        NiutrollerInterface oldNiutroller = niutroller;
        // Ensure invoke niutroller.isNiutroller() returns true
        require(newNiutroller.isNiutroller(), "marker method returned false");

        // Set market's niutroller to newNiutroller
        niutroller = newNiutroller;

        // Emit NewNiutroller(oldNiutroller, newNiutroller)
        emit NewNiutroller(oldNiutroller, newNiutroller);
...
contract UnitrollerAdminStorage {
    ...
    */
    address public pendingNiutrollerImplementation;
}

contract NiutrollerV1Storage is UnitrollerAdminStorage {
...
contract NiutrollerV2Storage is NiutrollerV1Storage {
    struct Market {
...
contract Unitroller is UnitrollerAdminStorage, NiutrollerErrorReporter {
    ...
    /**
      * @notice Emitted when pendingNiutrollerImplementation is changed
      */
    ...
    function _setPendingImplementation(address newPendingImplementation) public returns (uint) {
        ...
        pendingNiutrollerImplementation = newPendingImplementation;
    ...
    */
    function _acceptImplementation() public returns (uint) {
        ...
        if (msg.sender != pendingNiutrollerImplementation || pendingNiutrollerImplementation == address(0)) {
 ...
 * @author Niural
 */
contract NiutrollerG1 is NiutrollerV5Storage, NiutrollerInterface, NiutrollerErrorReporter, ExponentialNoError {
    ...
    /// @notice Emitted when a new NIU speed is calculated for a market
    event NiuSpeedUpdated(NToken indexed nToken, uint newSpeed);

    /// @notice Emitted when a new NIU speed is set for a contributor
    event ContributorNiuSpeedUpdated(address indexed contributor, uint newSpeed);

    /// @notice Emitted when NIU is distributed to a supplier
    event DistributedSupplierNiu(NToken indexed nToken, address indexed supplier, uint niuDelta, uint niuSupplyIndex);

    /// @notice Emitted when NIU is distributed to a borrower
    event DistributedBorrowerNiu(NToken indexed nToken, address indexed borrower, uint niuDelta, uint niuBorrowIndex);
    ...
    /// @notice Emitted when NIU is granted by admin
    event NiuGranted(address recipient, uint amount);

    /// @notice The initial NIU index for a market
    uint224 public constant niuInitialIndex = 1e36;
     ...
     */
    function enterMarkets(address[] memory nTokens) public override returns (uint[] memory) {
     ...
     */
    function exitMarket(address nTokenAddress) external override returns (uint) {
     ...
     */
    function mintAllowed(address nToken, address minter, uint mintAmount) external override returns (uint) {
     ...
     */
    function mintVerify(address nToken, address minter, uint actualMintAmount, uint mintTokens) external override {
        // Shh - currently unused
        nToken;
        minter;
        actualMintAmount;
     ...
     */
    function redeemAllowed(address nToken, address redeemer, uint redeemTokens) external override returns (uint) {
     ...
     */
    function redeemVerify(address nToken, address redeemer, uint redeemAmount, uint redeemTokens) override external {
        // Shh - currently unused
        nToken;
        redeemer;
     ...
     */
    function borrowAllowed(address nToken, address borrower, uint borrowAmount) external override returns (uint) {
        // Pausing is a very serious situation - we revert to sound the alarms
        ...
        if (!markets[nToken].accountMembership[borrower]) {
            ...
            Error err1 = addToMarketInternal(NToken(msg.sender), borrower);
        ...
        (Error err2, , uint shortfall) = getHypotheticalAccountLiquidityInternal(borrower, NToken(nToken), 0, borrowAmount);
     ...
     */
    function borrowVerify(address nToken, address borrower, uint borrowAmount) override external {
        // Shh - currently unused
        nToken;
        borrower;
        borrowAmount;
    ...
    function seizeAllowed(
        ...
        if (NToken(nTokenCollateral).niutroller() != NToken(nTokenBorrowed).niutroller()) {
            return uint(Error.NIUTROLLER_MISMATCH);
        }
     ...
     */
    function transferAllowed(address nToken, address src, address dst, uint transferTokens) external override returns (uint) {
        // Pausing is a very serious situation - we revert to sound the alarms
     ...
     * @param transferTokens The number of nTokens to transfer
     */
    function transferVerify(address nToken, address src, address dst, uint transferTokens) external override {
        // Shh - currently unused
        nToken;
        src;
        dst;
    ...
    function getHypotheticalAccountLiquidityInternal(
        ...
        for (uint i = 0; i < assets.length; i++) {
            ...
            // Pre-niuute a conversion factor from tokens -> ether (normalized price value)
    ...
    function _setPriceOracle(PriceOracle newOracle) public returns (uint) {
        ...
        // Track the old oracle for the niutroller
        PriceOracle oldOracle = oracle;

        // Set niutroller's oracle to newOracle
        oracle = newOracle;
    ...
    function adminOrInitializing() internal view returns (bool) {
        return msg.sender == admin || msg.sender == niutrollerImplementation;
    }
     ...
     * @param nToken The market whose NIU speed to update
     * @param niuSpeed New NIU speed for market
     */
    function setNiuSpeedInternal(NToken nToken, uint niuSpeed) internal {
        uint currentNiuSpeed = niuSpeeds[address(nToken)];
        if (currentNiuSpeed != 0) {
            // note that NIU speed could be set to 0 to halt liquidity rewards for a market
            ...
            // Add the NIU market
            Market storage market = markets[address(nToken)];
            require(market.isListed == true, "niu market is not listed");
        ...
        if (currentNiuSpeed != niuSpeed) {
            niuSpeeds[address(nToken)] = niuSpeed;
            emit NiuSpeedUpdated(nToken, niuSpeed);
        }
    }

    /**
     * @notice Accrue NIU to the market by updating the supply index
    ...
    function updateNiuSupplyIndex(address nToken) internal {
        ...
        uint supplySpeed = niuSpeeds[nToken];
    ...
    /**
     * @notice Accrue NIU to the market by updating the borrow index
    ...
    function updateNiuBorrowIndex(address nToken, Exp memory marketBorrowIndex) internal {
        ...
        uint borrowSpeed = niuSpeeds[nToken];
        uint blockNumber = getBlockNumber();
    ...
    /**
     * @notice Calculate NIU accrued by a supplier and possibly transfer it to them
     * @param nToken The market in which the supplier is interacting
     * @param supplier The address of the supplier to distribute NIU to
     */
    function distributeSupplierNiu(address nToken, address supplier) internal {
        ...
        if (supplierIndex.mantissa == 0 && supplyIndex.mantissa > 0) {
            supplierIndex.mantissa = niuInitialIndex;
        }
    ...
    /**
     * @notice Calculate NIU accrued by a borrower and possibly transfer it to them
     * @dev Borrowers will not begin to accrue until after the first interaction with the protocol.
     * @param nToken The market in which the borrower is interacting
     * @param borrower The address of the borrower to distribute NIU to
     */
    ...
    function updateContributorRewards(address contributor) public {
        uint niuSpeed = niuContributorSpeeds[contributor];
        uint blockNumber = getBlockNumber();
        uint deltaBlocks = sub_(blockNumber, lastContributorBlock[contributor]);
        if (deltaBlocks > 0 && niuSpeed > 0) {
            uint newAccrued = mul_(deltaBlocks, niuSpeed);
     ...
     * @param holder The address to claim NIU for
     */
     ...
     * @param holder The address to claim NIU for
     * @param nTokens The list of markets to claim NIU in
     */
     ...
     * @param holders The addresses to claim NIU for
     * @param nTokens The list of markets to claim NIU in
     * @param borrowers Whether or not to claim NIU earned by borrowing
     * @param suppliers Whether or not to claim NIU earned by supplying
     */
    ...
    /**
     * @notice Transfer NIU to the user
     * @dev Note: If there is not enough NIU, we do not perform the transfer all.
     * @param user The address of the user to transfer NIU to
     * @param amount The amount of NIU to (possibly) transfer
     * @return The amount of NIU which was NOT transferred to the user
     */
    function grantNiuInternal(address user, uint amount) internal returns (uint) {
        Niu niu = Niu(getNiuAddress());
        uint niuRemaining = niu.balanceOf(address(this));
        if (amount > 0 && amount <= niuRemaining) {
            niu.transfer(user, amount);
            return 0;
        }
        return amount;
    }
     ...
     * @dev Note: If there is not enough NIU, we do not perform the transfer all.
     * @param recipient The address of the recipient to transfer NIU to
     * @param amount The amount of NIU to (possibly) transfer
     */
    function _grantNiu(address recipient, uint amount) public {
        require(adminOrInitializing(), "only admin can grant niu");
        uint amountLeft = grantNiuInternal(recipient, amount);
        require(amountLeft == 0, "insufficient niu for grant");
     ...
     * @param niuSpeed New NIU speed for market
     */
    function _setNiuSpeed(NToken nToken, uint niuSpeed) public {
        require(adminOrInitializing(), "only admin can set niu speed");
        setNiuSpeedInternal(nToken, niuSpeed);
    }

    /**
     * @notice Set NIU speed for a single contributor
     * @param contributor The contributor whose NIU speed to update
     * @param niuSpeed New NIU speed for contributor
     */
    function _setContributorNiuSpeed(address contributor, uint niuSpeed) public {
        require(adminOrInitializing(), "only admin can set niu speed");

        // note that NIU speed could be set to 0 to halt liquidity rewards for a contributor
        ...
        niuContributorSpeeds[contributor] = niuSpeed;

        emit ContributorNiuSpeedUpdated(contributor, niuSpeed);
    }
     ...
     */
    function getNiuAddress() public pure returns (address) {
