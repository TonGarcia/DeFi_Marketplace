...
contract Niutroller is NiutrollerV7Storage, NiutrollerInterface, NiutrollerErrorReporter, ExponentialNoError {
    ...
    /// @notice Emitted when a new borrow-side NIU speed is calculated for a market
    event NiuBorrowSpeedUpdated(NToken indexed nToken, uint newSpeed);

    /// @notice Emitted when a new supply-side NIU speed is calculated for a market
    ...
    /// @notice Emitted when NIU accrued for a user has been manually adjusted.
    event NiuAccruedAdjusted(address indexed user, uint oldNiuAccrued, uint newNiuAccrued);

    /// @notice Emitted when NIU receivable for a user has been updated.
    ...
    function _initializeMarket(address nToken) internal {
        ...
        if (supplyState.index == 0) {
            ...
            supplyState.index = niuInitialIndex;
        }

        if (borrowState.index == 0) {
            // Initialize borrow state index with default value
            borrowState.index = niuInitialIndex;
        }
     ...
     * @param supplySpeed New supply-side NIU speed for market
     * @param borrowSpeed New borrow-side NIU speed for market
     */
    function setNiuSpeedInternal(NToken nToken, uint supplySpeed, uint borrowSpeed) internal {
        Market storage market = markets[address(NToken)];
        require(market.isListed, "niu market is not listed");

        if (niuSupplySpeeds[address(NToken)] != supplySpeed) {
            // Supply speed updated so let's update supply state to ensure that
            //  1. NIU accrued properly for the old speed, and
            //  2. NIU accrued at the new speed starts after this block.
     ...
     * @dev Index is a cumulative sum of the NIU per NToken accrued.
     */
    ...
    function distributeSupplierNiu(address nToken, address supplier) internal {
        // TODO: Don't distribute supplier NIU if the user is not in the supplier market.
        ...
        // Update supplier's index to the current index since we are distributing accrued NIU
        niuSupplierIndex[NToken][supplier] = supplyIndex;

        if (supplierIndex == 0 && supplyIndex >= niuInitialIndex) {
            // Covers the case where users supplied tokens before the market's supply state index was set.
            // Rewards the user with NIU accrued from the start of when supplier rewards were first
            // set for the market.
            supplierIndex = niuInitialIndex;
        }

        // Calculate change in the cumulative sum of the NIU per NToken accrued
        Double memory deltaIndex = Double({mantissa: sub_(supplyIndex, supplierIndex)});

        uint supplierTokens = NToken(NToken).balanceOf(supplier);

        // Calculate NIU accrued: NTokenAmount * accruedPerNToken
    ...
    function distributeBorrowerNiu(address nToken, address borrower, Exp memory marketBorrowIndex) internal {
        // TODO: Don't distribute supplier NIU if the user is not in the borrower market.
        ...
        // Update borrowers's index to the current index since we are distributing accrued NIU
        niuBorrowerIndex[NToken][borrower] = borrowIndex;

        if (borrowerIndex == 0 && borrowIndex >= niuInitialIndex) {
            // Covers the case where users borrowed tokens before the market's borrow state index was set.
            // Rewards the user with NIU accrued from the start of when borrower rewards were first
            // set for the market.
            borrowerIndex = niuInitialIndex;
        }

        // Calculate change in the cumulative sum of the NIU per borrowed unit accrued
        Double memory deltaIndex = Double({mantissa: sub_(borrowIndex, borrowerIndex)});

        uint borrowerAmount = div_(NToken(NToken).borrowBalanceStored(borrower), marketBorrowIndex);
        
        // Calculate NIU accrued: NTokenAmount * accruedPerBorrowedUnit
    ...
    /**
     * @notice Set NIU borrow and supply speeds for the specified markets.
     * @param NTokens The markets whose NIU speed to update.
     * @param supplySpeeds New supply-side NIU speed for the corresponding market.
     * @param borrowSpeeds New borrow-side NIU speed for the corresponding market.
     */
