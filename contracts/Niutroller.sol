...
contract Niutroller is NiutrollerV7Storage, NiutrollerInterface, NiutrollerErrorReporter, ExponentialNoError {
    ...
    event NiuBorrowSpeedUpdated(CToken indexed cToken, uint newSpeed);

    /// @notice Emitted when a new supply-side COMP speed is calculated for a market
    event NiuSupplySpeedUpdated(CToken indexed cToken, uint newSpeed);
    ...
    function mintAllowed(address cToken, address minter, uint mintAmount) external returns (uint) {
        ...
        distributeSupplierNiu(cToken, minter);
    ...
    function redeemAllowed(address cToken, address redeemer, uint redeemTokens) external returns (uint) {
        ...
        // Keep the flywheel moving
        updateNiuSupplyIndex(cToken);
        distributeSupplierNiu(cToken, redeemer);
    ...
    function borrowAllowed(address cToken, address borrower, uint borrowAmount) external returns (uint) {
        ...
        distributeBorrowerNiu(cToken, borrower, borrowIndex);
    ...
    function seizeAllowed(
        ...
        distributeSupplierNiu(cTokenCollateral, borrower);
        distributeSupplierNiu(cTokenCollateral, liquidator);
    ...
    function transferAllowed(address cToken, address src, address dst, uint transferTokens) external returns (uint) {
        ...
        // Keep the flywheel moving
        updateNiuSupplyIndex(cToken);
        distributeSupplierNiu(cToken, src);
        distributeSupplierNiu(cToken, dst);
     ...
     */
    function setNiuSpeedInternal(CToken cToken, uint supplySpeed, uint borrowSpeed) internal {
        Market storage market = markets[address(cToken)];
        ...
        if (compSupplySpeeds[address(cToken)] != supplySpeed) {
            ...
            emit NiuSupplySpeedUpdated(cToken, supplySpeed);
        }
        ...
        if (compBorrowSpeeds[address(cToken)] != borrowSpeed) {
            ...
            emit NiuBorrowSpeedUpdated(cToken, borrowSpeed);
        }
    }
     ...
     */
    function distributeSupplierNiu(address cToken, address supplier) internal {
        ...
        emit DistributedSupplierNiu(CToken(cToken), supplier, supplierDelta, supplyIndex);
    }
     ...
     */
    function distributeBorrowerNiu(address cToken, address borrower, Exp memory marketBorrowIndex) internal {
        ...
        emit DistributedBorrowerNiu(CToken(cToken), borrower, borrowerDelta, borrowIndex);
    }
        ...
        for (uint i = 0; i < cTokens.length; i++) {
            ...
            if (borrowers == true) {
                ...
                for (uint j = 0; j < holders.length; j++) {
                    distributeBorrowerNiu(address(cToken), holders[j], borrowIndex);
                }
            }
            if (suppliers == true) {
                updateNiuSupplyIndex(address(cToken));
                for (uint j = 0; j < holders.length; j++) {
                    distributeSupplierNiu(address(cToken), holders[j]);
                }
            }
        }
        for (uint j = 0; j < holders.length; j++) {
     ...
     */
    function _setNiuSpeeds(CToken[] memory cTokens, uint[] memory supplySpeeds, uint[] memory borrowSpeeds) public {
        ...
        for (uint i = 0; i < numTokens; ++i) {
            setNiuSpeedInternal(cTokens[i], supplySpeeds[i], borrowSpeeds[i]);
        }
    }
