 ...
 * @author Niural
 * @dev This was the first version of the Niutroller brains.
 *  We keep it so our tests can continue to do the real-life behavior of upgrading from this logic forward.
 */
contract NiutrollerG1 is NiutrollerV1Storage, NiutrollerInterface, NiutrollerErrorReporter, Exponential {
    struct Market {
        /**
    ...
    function enterMarkets(address[] memory nTokens) public returns (uint[] memory) {
        ...
        for (uint i = 0; i < len; i++) {
            ...
            accountAssets[msg.sender].push(nToken);

            emit MarketEntered(nToken, msg.sender);
    ...
    function liquidateBorrowVerify(
        ...
        nTokenBorrowed;   // currently unused
        nTokenCollateral; // currently unused
        liquidator;       // currently unused
        borrower;         // currently unused
        repayAmount;      // currently unused
     ...
     * @param repayAmount The amount of nTokenBorrowed underlying to convert into nTokenCollateral tokens
     * @return (errorCode, number of nTokenCollateral tokens to be seized in a liquidation)
     */
    function liquidateCalculateSeizeTokens(address nTokenBorrowed, address nTokenCollateral, uint repayAmount) external view returns (uint, uint) {
    ...
    function _supportMarket(NToken nToken) external returns (uint) {
        ...
        markets[address(nToken)] = Market({isListed: true, collateralFactorMantissa: 0});
    ...
    function _become(Unitroller unitroller, PriceOracle _oracle, uint _closeFactorMantissa, uint _maxAssets, bool reinitializing) public {
        ...
        if (!reinitializing) {
            NiutrollerG1 freshBrainedNiutroller = NiutrollerG1(address(unitroller));

            // Ensure invoke _setPriceOracle() = 0
            uint err = freshBrainedNiutroller._setPriceOracle(_oracle);
            require (err == uint(Error.NO_ERROR), "set price oracle error");

            // Ensure invoke _setCloseFactor() = 0
            err = freshBrainedNiutroller._setCloseFactor(_closeFactorMantissa);
            require (err == uint(Error.NO_ERROR), "set close factor error");

            // Ensure invoke _setMaxAssets() = 0
            err = freshBrainedNiutroller._setMaxAssets(_maxAssets);
            ...
            err = freshBrainedNiutroller._setLiquidationIncentive(liquidationIncentiveMinMantissa);
