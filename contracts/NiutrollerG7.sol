 ...
 * @author Niural
 */
contract NiutrollerG7 is NiutrollerV5Storage, NiutrollerInterface, NiutrollerErrorReporter, ExponentialNoError {
     ...
     */
    function setNiuSpeedInternal(CToken cToken, uint compSpeed) internal {
        uint currentNiuSpeed = compSpeeds[address(cToken)];
        if (currentNiuSpeed != 0) {
            ...
            if (compSupplyState[address(cToken)].index == 0 && compSupplyState[address(cToken)].block == 0) {
                compSupplyState[address(cToken)] = NiuMarketState({
            ...
            if (compBorrowState[address(cToken)].index == 0 && compBorrowState[address(cToken)].block == 0) {
                compBorrowState[address(cToken)] = NiuMarketState({
                    index: compInitialIndex,
                    block: safe32(getBlockNumber(), "block number exceeds 32 bits")
                });
            }
        ...
        if (currentNiuSpeed != compSpeed) {
            compSpeeds[address(cToken)] = compSpeed;
            emit NiuSpeedUpdated(cToken, compSpeed);
        }
    }
     ...
     * @param compSpeed New COMP speed for market
     */
    function _setNiuSpeed(CToken cToken, uint compSpeed) public {
        require(adminOrInitializing(), "only admin can set comp speed");
        setNiuSpeedInternal(cToken, compSpeed);
    }
