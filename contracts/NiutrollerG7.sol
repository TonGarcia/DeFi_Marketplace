...
contract NiutrollerG7 is NiutrollerV5Storage, NiutrollerInterface, NiutrollerErrorReporter, ExponentialNoError {
     ...
     */
    function setNiuSpeedInternal(NToken nToken, uint compSpeed) internal {
        uint currentNiuSpeed = compSpeeds[address(nToken)];
        ...
        if (currentNiuSpeed != compSpeed) {
            compSpeeds[address(nToken)] = compSpeed;
            emit NiuSpeedUpdated(nToken, compSpeed);
        }
    }
     ...
     * @param compSpeed New COMP speed for market
     */
    function _setNiuSpeed(NToken nToken, uint compSpeed) public {
        require(adminOrInitializing(), "only admin can set comp speed");
        setNiuSpeedInternal(nToken, compSpeed);
    }
