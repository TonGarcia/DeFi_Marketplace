...
import { getNiutroller, getCTokenData } from '../ContractLookup';
...
async function setNiutroller(world: World, from: string, cToken: CToken, comptroller: string): Promise<World> {
  let invokation = await invoke(world, cToken.methods._setNiutroller(comptroller), from, CTokenErrorReporter);
...
export function cTokenCommands() {
  ...
  `,
    ...
    new Command<{ cToken: CToken, comptroller: AddressV }>(`
      ...
      (world, from, { cToken, comptroller }) => setNiutroller(world, from, cToken, comptroller.val),
      { namePos: 1 }
    ),
    new Command<{
      cToken: CToken;
